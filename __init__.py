...
class AprsParser(PickleModule):
    ...
    def parseRegularAprsData(self, information):
        # CA2RXU LoRa APRS Base91 position
        #
        # Format after the APRS '='/'!' DTI:
        #   overlay + 4-char latitude + 4-char longitude + symbol + ...
        #
        # Example:
        #   L<Rk8TU&a;...
        #
        # CA2RXU uses the standard APRS Base91 coordinate formulas,
        # but without the normal APRS compressed-coordinate prefix.

        if (
            len(information) >= 10
            and information[4] != "."
            and len(information[1:5]) == 4
            and len(information[5:9]) == 4
            and all(33 <= ord(c) <= 123 for c in information[1:9])
        ):
            aprsData = {
                "lat": 90 - decodeBase91(information[1:5]) / 380926,
                "lon": -180 + decodeBase91(information[5:9]) / 190463,
                "symbol": getSymbolData(information[9], information[0]),
                "type": "compressed"
            }

            # CA2RXU course/speed/altitude data follows the symbol.
            #
            # For the currently received packet:
            #   L<Rk8TU&a; a !
            #
            # There is no standard APRS compressed extension to decode
            # here, so leave the position data intact.

            # CA2RXU compressed position data after the symbol:
            #
            # Normal:
            #   [10] = course
            #   [11] = speed
            ...
            #   [12] = compression/type
            #
            # The human-readable comment starts at [13].
            #
            # Example:
            #   L<Rk8TU&;a !GLoRa APRS Reciever 433.775 Mhz powered by ...
            #             ^ ^ ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            #             | | comment starts here
            #             | CA2RXU type
            #             speed
            ...
            if len(information) > 12:
                if information[10] == " ":
                    ...
                    # CA2RXU encoded speed.
                    # '!' = zero speed.
                    aprsData["speed"] = (
                        1.08 ** (ord(information[11]) - 33) - 1
                    ) * knotsToKilometers

                elif information[12] == "Q":
                    # CA2RXU encoded altitude
                    aprsData["course"] = 0
                    aprsData["speed"] = 0

                    aprsData["altitude"] = (
                        1.002 ** (
                            (ord(information[10]) - 33) * 91
                            + (ord(information[11]) - 33)
                        )
                    ) * 0.3048

                else:
                    # CA2RXU encoded course and speed
                ...
                # [12] is the CA2RXU type/compression byte ('G').
                # It is NOT part of the comment.
                comment = information[13:]

                # Remove CA2RXU trailing data after '|'
                if "|" in comment:
                    comment = comment.split("|", 1)[0]
            else:
                comment = ""

        elif self.hasCompressedCoordinates(information):
            ...
            if information[10] != " ":
                ...
                # Remove CA2RXU application/control data.
                if "|" in comment:
                    comment = comment.split("|", 1)[0]
        else:
